...
// 实参是非指针，则形参会复制实参的值，然后在函数中使用，所以函数中对形参的操作不会影响到实参
// 实参是指针，则形参会赋值实参的指针，然后在函数中使用，所以函数中对形参的操作会影响到实参
func func17(a int, b *int) {
	// 注意：这里实参 b 指针的地址和形参 b 指针的地址是相同的，但是实参 b 指针的指针的地址和形参 b 指针的指针的地址是不同的（也就是说不管是不是指针，形参都会将实参复制出一份并使用）
